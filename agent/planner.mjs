...
NARRATION — decide from the film's FORM whether a voiceover narrator serves the story. Choose "voiceover" when the form is lifted by a narrator's voice: a film-noir detective tale (first person, hardboiled, past tense), a nature or expository documentary (omniscient), a memoir, fable, or storybook (reflective, intimate). Choose "none" for dialogue- or action-driven pieces where narration would merely describe what we already see. When mode is "voiceover", write each scene's "narration" as ONE or TWO short sentences in that voice that give the viewer the EXPOSITION needed to follow the story — who we are watching, where we are, what is at stake, what just changed — and carry the through-line forward; never just narrate the visible action. Pick a fitting "voice". When mode is "none", set every scene "narration" to "".
