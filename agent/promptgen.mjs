...
- NO NEW ELEMENTS: introduce no new named props, vehicles, signage, labels, logos, or secondary characters beyond those named in the STORY BEAT, the REQUIRED IN FRAME list, or the LOCKED CONTINUITY FACTS. Keep incidental background generic and unbranded so nothing can drift between shots. NEVER add background passersby or bystanders unless the beat names them — an empty background cannot pop in and out between shots.
- SETTING LOCK: explicitly restate the location's terrain/architecture, the WEATHER, and the TIME OF DAY from SETTING in EVERY image_prompt, using the same wording across all shots of the scene — the place, sky, and light must be identical shot to shot, only the camera and the action change.
